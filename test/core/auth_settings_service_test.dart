...
void main() {
  ...
  group('isAuthEnabled / setAuthEnabled', () {
    test('returns false when key is absent', () async {
      when(
        () => storage.read(key: 'vitalglyph_auth_enabled'),
    ...
    test('setAuthEnabled writes "true"/"false" string', () async {
      ...
      verify(
        () => storage.write(key: 'vitalglyph_auth_enabled', value: 'true'),
      ).called(1);

      await service.setAuthEnabled(enabled: false);
      verify(
        () => storage.write(key: 'vitalglyph_auth_enabled', value: 'false'),
      ).called(1);
    });
  });
  ...
  group('isBiometricEnabled / setBiometricEnabled', () {
    test('returns false when key is absent', () async {
      when(
        () => storage.read(key: 'vitalglyph_biometric_enabled'),
      ).thenAnswer((_) async => null);
    ...
    test('setBiometricEnabled writes correct string', () async {
      ...
      verify(
        () => storage.write(key: 'vitalglyph_biometric_enabled', value: 'true'),
      ).called(1);
    });
  });
  ...
  group('getLockTimeout / setLockTimeout', () {
    test('returns LockTimeout.immediately when key is absent', () async {
      when(
        () => storage.read(key: 'vitalglyph_lock_timeout'),
      ).thenAnswer((_) async => null);
    ...
    test('setLockTimeout writes enum name', () async {
      ...
      verify(
        () => storage.write(key: 'vitalglyph_lock_timeout', value: 'after1Min'),
      ).called(1);
    });
  });
}
